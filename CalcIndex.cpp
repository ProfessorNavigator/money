...
CalcIndex::mainCalc ()
{
  ...
  operationName.emit (gettext ("File list creation:"));
  ...
  for (size_t i = 0; i < maindir.size (); i++)
      ...
      for (auto &p : std::filesystem::directory_iterator (maindir[i]))
	{
      ...
      progr.emit ((double) i / (double) maindir.size ());
    }

  progr.emit (0);
  operationName.emit (gettext ("Data collection:"));
  ...
  for (size_t i = 0; i < filelist.size (); i++)
      ...
      while (!f.eof ())
	{
	  ...
	  if (linecount > 1 && line != "")
	    {
	      ...
	      tmp.erase (0, tmp.find (".") + std::string (".").size ());
	      year = std::stoi (tmp);
	      date = af.grigtojulian (day, month, year);
	      std::locale loc ("C");
	      ...
	      strm << moneystr;
	      strm >> money;
	      if (date < af.grigtojulian (1, 1, 1998))
		{
		  money = money * 0.001;
		}
      ...
      f.close ();
      progr.emit ((double) i / (double) filelist.size ());
    }
  maindir.clear ();
  filelist.clear ();
  std::sort (
  ...
  operationName.emit (gettext ("Result saving:"));
  progr.emit (0);
  for (size_t i = 0; i < data.size (); i++)
      ...
      if (iter != infdates.end ())
	{
      ...
      progr.emit ((double) i / (double) data.size ());
    }
  f.close ();
  af.homePath (&dirline);
