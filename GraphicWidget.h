...
private:
  ...
  on_draw (const Cairo::RefPtr<Cairo::Context> &cr, int width, int height,
	   std::string file, mglGraph *graph, int variant);
  void
  on_drawedit (const Cairo::RefPtr<Cairo::Context> &cr, int width, int height,
	       mglGraph *graph);
  void
  zoomGraph (mglGraph *gr, Gtk::DrawingArea *drar, int id);
  void
  saveGraph (mglGraph *gr, int mode);
  void
  saveJPEG (Gtk::Window *window, std::string filename, Gtk::FileChooser *filech,
	    Gtk::Entry *file, mglGraph *gr, int mode);
  bool
  scrollEvent (double x, double y, mglGraph *gr, Gtk::DrawingArea *drar);
  void
  dragOperation (double x, double y, mglGraph *gr, Gtk::DrawingArea *drar);
  void
