...
GraphicWidget::graphic(std::string file, int graphvar)
{
  ...
  scr->signal_scroll().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::scrollEvent),
               drar),
    true);
  ...
  drag->signal_drag_update().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::dragOperation),
               drar));
  ...
  increase->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::zoomGraph), drar,
               0));
  ...
  savejpg->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::saveGraph), 0));
  ...
  savepng->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::saveGraph), 1));
  ...
  cl->signal_pressed().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::rightClick), cl,
               drar));
...
void
GraphicWidget::saveGraph(int mode)
{
  std::string filename;
  Gtk::Window *window = new Gtk::Window;
  Gtk::Grid *grid = Gtk::make_managed<Gtk::Grid> ();
  ...
  save->signal_clicked().connect(
    sigc::bind(sigc::mem_fun(*this, &GraphicWidget::saveJPEG), window,
               filename, filech, file, mode));
}

void
GraphicWidget::saveJPEG(Gtk::Window *window, std::string filename,
                        Gtk::FileChooser *filech, Gtk::Entry *file, int mode)
{
  ...
  if(std::filesystem::exists(*p))
    {
      ...
      dialog->show();
      dialog->signal_response().connect([dialog, p, window, this, mode]
                                        (int id)
      {
        if(id == 1)
          {
        ...
        if(id == 0)
          {
            ...
            if(mode == 0)
              {
                this->gr->WriteJPEG(p->u8string().c_str());
              }

            if(mode == 1)
              {
                this->gr->WritePNG(p->u8string().c_str());
              }
            dialog->close();
            delete dialog;
            delete p;
